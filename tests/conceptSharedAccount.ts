...
// Bottleneck: Total time: 24839.018251 ms
// Bottleneck: Average time per donation: 82.79672750333334 ms
//     ✔ Exploring Bottleneck  (24840ms)
// Splitting: Total time donating: 2735.739292 ms
// Splitting: Average time per call: 9.119130973333334 ms
//     ✔ Exploring No Bottleneck (2736ms)
// Splitting: Total time donating: 1409.595458 ms
// Splitting: Total time redeeming: 20180.306209000002 ms
// Splitting: Total time: 21589.901667000002 ms
// Splitting: Average time per donating: 4.698651526666667 ms
// Splitting: Average time per redeeming: 67.26768736333334 ms
//     ✔ Exploring No Bottleneck With Redeeming (21590ms)
...
describe("Concept Shared Account", () => {
  ...
  const communityWallet = new anchor.web3.PublicKey("JD5C5Bsp3q9jeC5S57QuSCDDfpeKzXvRkfPB3Td6x3Wh");
  ...
  console.log("RUNNING TESTS with ", RUNS, " runs and ", REPETITIONS, " repetitions")
  ...
  it("Exploring Bottleneck ", async () => {
    ...
    for (let y = 0; y < repetitions; y++) {
      ...
      const promises = Array.from({length: runs}, async (_, i) => {
        ...
        return tempPrograms[i].methods.conceptSharedAccountBottleneck(
          amountToDonate
        ).accounts({
  ...
  it("Exploring No Bottleneck", async () => {
    ...
    for (let y = 0; y < repetitions; y++) {
      ...
      const donatePromise = Array.from({length: runs}, async (_, i) => {
        ...
        await tempPrograms[i].methods.conceptSharedAccount(
          amountToDonate
        ).accounts({
          owner: tempPrograms[i].provider.publicKey,
          donationTally: donationTallyAccount,
  ...
  it("Exploring No Bottleneck With Redeeming", async () => {
    ...
    for (let y = 0; y < repetitions; y++) {
      ...
      const redeemPromises = Array.from({length: runs}, async (_, i) => {
        ...
        await tempPrograms[i].methods.conceptSharedAccountRedeem().accounts({
          owner: tempPrograms[i].provider.publicKey,
          donationTally: donationTallyAccount,
          communityWallet: communityWallet,
          systemProgram: anchor.web3.SystemProgram.programId,
...
function getDonationTallyAccountKey(program: anchor.Program<Architecture>, owner?: anchor.web3.PublicKey){
  
  const [donationTallyAccount, _bump] = anchor.web3.PublicKey.findProgramAddressSync(
...
async function getTempPrograms(program: anchor.Program<Architecture>, runs: number){
  const programPromise: Promise<Program<Architecture>>[] = Array.from({length: runs}, async (_, i) => {
    const payer = anchor.web3.Keypair.generate();
    await program.provider.connection.requestAirdrop(payer.publicKey, anchor.web3.LAMPORTS_PER_SOL)
    ...
    return new anchor.Program<Architecture>(IDL, program.programId, tempProvider);
  })
