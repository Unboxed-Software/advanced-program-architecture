...
describe("Concept Indexing", () => {
  ...
  it("Exploring Indexing", async () => {
    ...
    const [multiplePerOwner] = anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from("MULTIPLE PER OWNER"), wallet.publicKey.toBuffer(), multiplePerOwnerIndex.toBuffer("be", 8)],
      program.programId
    );

    const someAccountToIndexOffOf = anchor.web3.Keypair.generate();
    const [onePerOwnerPerAccount] = anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from("ONE PER OWNER PER ACCOUNT"), wallet.publicKey.toBuffer(), someAccountToIndexOffOf.publicKey.toBuffer()],
      program.programId
    );
    ...
    const txHash = await program.methods.conceptIndexing(
      multiplePerOwnerIndex
    ).accounts({
